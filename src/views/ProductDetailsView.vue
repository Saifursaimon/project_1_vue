<template>
    <div class="py-12">
        ...
        <div v-else-if="state.product">
            ...
            <div class="mt-12">
                <div v-for="(d, index) in state.product.documents" :key="index"
            ...
            <div class=" mt-12 flex items-start gap-10">
                ...
                <div class="flex flex-col gap-6">
                    <a v-for="(c, i) in state.product.contacts" :key="i"
                        :href="c.startsWith('http') ? c : `https://${c}`" target="blank"
                        class=" text-lg underline underline-offset-6 decoration-2 decoration-[#2368F0]">
                        {{ c }}
                    </a>
                </div>
            </div>
...
onMounted(() => {
    ...
    state.product = productsData.find(p => p.id == productId)
